...
export const donjonsApi = {
  ...
  enter: (donjonId: number, data: { groupeId?: number; personnageId?: number; difficulte: number }) =>
    api.post(`/donjons/${donjonId}/enter`, data).then(r => r.data),
  getRunState: (groupeId: number) =>
    api.get(`/donjons/run/${groupeId}`).then(r => r.data),
  getRunStateSolo: (charId: number) =>
    api.get(`/donjons/run/solo/${charId}`).then(r => r.data),
  abandonRun: (groupeId: number) =>
    api.post(`/donjons/run/${groupeId}/abandon`).then(r => r.data),
  abandonRunSolo: (charId: number) =>
    api.post(`/donjons/run/solo/${charId}/abandon`).then(r => r.data),
